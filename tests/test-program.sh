...
GMSK_TRANSFER=../src/gmsk-transfer
...
check_ok_io()
{
    ...
    echo "Test: ${NAME}"
    ${GMSK_TRANSFER} -t -r io ${OPTIONS1} ${MESSAGE} > ${SAMPLES}
    ${GMSK_TRANSFER} -r io ${OPTIONS2} ${DECODED} < ${SAMPLES}
...
check_ok_file()
{
    ...
    echo "Test: ${NAME}"
    ${GMSK_TRANSFER} -t -r file=${SAMPLES} ${OPTIONS1} ${MESSAGE}
    ${GMSK_TRANSFER} -r file=${SAMPLES} ${OPTIONS2} ${DECODED}
    diff -q ${MESSAGE} ${DECODED} > /dev/null
}
